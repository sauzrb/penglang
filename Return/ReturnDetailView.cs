...
namespace PengLang
{
    public class ReturnDetailView : BaseBandView
    {
        ...
        public ReturnDetailView()
        {
            this.LoadData = this.LoadDataRows;
        }

        public ReturnDetailView(BandedGridView view)
        {
        ...
        protected override void InitGridView()
        {
            ...
            gridView.ValidateRow += new ValidateRowEventHandler(OnValidateRow);
        }
        ...
        protected override void CreateGridColumns()
        {
            ...
            // colCutNo
            // 
            this.colCutNo.Caption = "Color";//"Cut#";
            this.colCutNo.FieldName = "Color";//"StyleNo";
            this.colCutNo.Name = "colCutNo";
            this.colCutNo.UnboundType = DevExpress.Data.UnboundColumnType.String;
            this.colCutNo.Visible = true;
            this.colCutNo.Width = 80;
            this.colCutNo.OptionsColumn.AllowEdit = false;
            // 
            // colLotNo
            // 
        ...
        protected override void BindingBandColumn()
        {
            ...
            view.Columns["Color"].OwnerBand = bandCutNo;
        ...
        public void SetGridEditStatus(bool bEdit)
        {
            ...
            for (int i = 2; i <= EndSizeNoIndex; i++)
            {

                this.Columns[i].OptionsColumn.AllowEdit = bEdit;
            }
            colUnitPrice.OptionsColumn.AllowEdit = bEdit;
        ...
        protected  DataTable LoadDetail(string boundID)
        {
            DataTable dt = null;

            string sql = String.Format("Select DetailID as RowID, LotNo, SizeNo, Quantity, Funds  From T_BackboundDetail where BackboundID = '{0}' and Quantity > 0", boundID);
        ...
        protected void LoadDataRows()
        {
            List<InvoiceItem> list = InvoiceList;
            list.Clear();
            DataTable dt = LoadDetail(BackboundID);
        ...
        public void OnCellValueChanged(object sender, CellValueChangedEventArgs e)
        {
            ...
            else if (e.Column.AbsoluteIndex >= BeginSizeNoIndex && e.Column.AbsoluteIndex <= EndSizeNoIndex)
            {
                ...
                gridView.SetFocusedRowCellValue(colQuantity, sum);

                Object obj = gridView.GetRowCellValue(e.RowHandle, colUnitPrice);
            ...
            {
                Object obj = gridView.GetRowCellValue(e.RowHandle, colQuantity);
                if (obj == null || string.IsNullOrEmpty(obj.ToString()))
                    quantity = 0;
        ...
        public void OnValidateRow(object sender, ValidateRowEventArgs e)
        {
            ...
            if (GetSubTotal(e.RowHandle) <= 0)
            {
                e.ErrorText = "QTY can not be empty. ";
                e.Valid = false;
                return;
            } 
        ...
        private void AutoCompleteClothes(string lotno)
        {
            ...
            if (clothes == null)
                return;
            //gridView.SetFocusedRowCellValue(colCutNo, clothes.StyleNo);
            gridView.SetFocusedRowCellValue(colCutNo, clothes.Color);
        }
        ...
        public void RemoveRows(int[] rowHandles)
        {
            ...
            for (int i = rowHandles.Length - 1; i >= 0; i--)
            {
                ...
                if (objDetail != null)
                {
                    RemoveBackboundDetail(objDetail.ToString());
                }
                gridView.DeleteRow(rowHandles[i]);
            }
        }

        public bool RemoveBackboundDetail(string detialID)
        {
            string sql = String.Format("delete from T_BackboundDetail where detailID = '{0}'", detialID);
            return Database.ExecuteSQL(sql, "ReturnDetailView-RemoveBackboundDetail") == 0 ? false : true;
        }
        ...
        public void SaveBackboundDetail()
        {
            DetailIndex = 0;
        ...
        private void SaveBackboundRow(int idx)
        {
            ...
            string sizeNo = "";
            int quantity = 0; 
            string detailID = Database.GetDataTimekey(4, DetailIndex++);
            gridView.SetRowCellValue(idx, colRowID, detailID);
           
            double price =  0.0  ;

            Object obj = null;
            obj = gridView.GetRowCellValue(idx, colUnitPrice);
            ...
            for (int i = BeginSizeNoIndex; i <=EndSizeNoIndex; i++)
            {
                ...
                if (quantity <= 0)
                    continue;
                SaveBackboundCell(detailID, lotno, sizeNo, quantity, price, 0 );
            }
        }

        private void SaveBackboundCell(string detailID, string lotNo, string sizeNo, int quantity, double price, double woprice)
        {

            string sql = String.Format("Insert into T_BackboundDetail (DetailID, BackboundID, LotNo, SizeNo, Quantity ,funds ,WOPrice, Status ) "
            ...
            Database.ExecuteSQL(sql, "ReturnDetailView-RemoveBackboundDetail");
        }
